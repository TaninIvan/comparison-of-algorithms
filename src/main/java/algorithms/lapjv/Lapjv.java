package algorithms.lapjv;

import algorithms.Algorithm;

public class Lapjv implements Algorithm {

    private double[][] costs;

    public Lapjv(double[][] costs) {
        this.costs = new double[costs.length][costs[0].length];
        for (int i=0;i < costs.length;i++)
            System.arraycopy(costs[i], 0, this.costs[i], 0, costs[0].length);
    }
    ...
    public int[][] execute()
    {
        ...
        int rdim = costs.length;
        int cdim = costs[0].length;
        costs = MatrixUtils.makeSquare(costs,MatrixUtils.sum(costs)+1.0);
        int dim = costs.length;

        rowsol = new int[dim];
        colsol = new int[dim];
        v = new double[costs[0].length];
        ...
        int[][] result = new int[sol.length][2];
    ...
    public String getName() {
        return "Joncker-Volgenant Algorithm";
    }
