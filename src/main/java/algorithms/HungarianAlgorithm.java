...
public class HungarianAlgorithm implements Algorithm {
    ...
    public HungarianAlgorithm(double[][] theCosts) throws Exception {
        costs = new double[theCosts.length][theCosts[0].length];
        for (int i=0;i < theCosts.length;i++)
            System.arraycopy(theCosts[i], 0, costs[i], 0, theCosts[0].length);
